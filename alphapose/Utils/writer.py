...
class DataWriter():
    ...
    def save(self, boxes, scores, ids, hm_data, cropped_boxes, orig_img, im_name):
        ...
        # print("boxes shape is {0} scores shape is {1} ids shape is {2} hm_data shape is {3} cropped box shape is {4}".format(boxes.shape, scores.shape, ids.shape, hm_data.shape, cropped_boxes.shape))
        # print("boxes shape is {0} scores shape is {1} ids shape is {2} hm_data shape is {3} cropped box shape is {4}".format(boxes.shape, scores.shape, ids.shape, hm_data.shape, cropped_boxes.shape))
...
class DataWriter_rs():
    def __init__(self, cfg, opt, save_video=False,
        ...
        if opt.sp:
            self.result_queue = Queue(maxsize=queueSize)
            self.key_points = Queue(maxsize=queueSize)
        else:
            self.result_queue = mp.Queue(maxsize=queueSize)
            self.key_points = mp.Queue(maxsize=queueSize)
    ...
    def get_keypoints(self):
        return self.wait_and_get(self.key_points)
    ...
    def update(self):
        ...
        while True:
            ...
            if boxes is None or len(boxes) == 0:
                ...
                img = vis_frame(orig_img, result, self.opt, self.vis_thres)
                if len(_result) > 0:    
                    self.wait_and_put(self.key_points, (_result[0], img))
                if self.opt.vis:
                    self.show_image(img, im_name, stream=stream if self.save_video else None)


                

    def show_image(self, img, im_name, stream=None):
        if self.opt.vis:
            cv2.imshow("AlphaPose Demo", img)
            cv2.waitKey(5)
        if self.save_video:
