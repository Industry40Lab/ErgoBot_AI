...
```
> ros2 launch llm_supervisor llm_supervisor.launch.py
```
...
- run `ros2 run pal_deploy deploy --package llm_supervisor generic-XXc` (replace `generic-XXc` by your actual robot)

You can now `ssh` onto the robot (`ssh pal@generic-XXc`, password
`pal`), and go to your project: `cd ~/deployed_ws/share/llm_supervisor`
...
`ros2 run llm_supervisor run_app`
...
``ros2 lifecycle set /app_llm_supervisor configure``
``ros2 lifecycle set /app_llm_supervisor activate``

To stop it (but not reseting it's internal variables):
``ros2 lifecycle set /app_llm_supervisor activate``

To stop it and clean it:
``ros2 lifecycle set /app_llm_supervisor deactivate``
``ros2 lifecycle set /app_llm_supervisor cleanup``
