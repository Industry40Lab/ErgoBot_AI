...
from notif_task_msgs.action import Notif
...
STATES = {
    Notif.Goal().INACTIVE: "INACTIVE",
    Notif.Goal().Action_Notif: "Action_Notif",
    Notif.Goal().Danger_Notif: "Danger_Notif",
}


class TaskImpl(Node):
    """
    Implementation of task notif_task.
    ...
    A real 'notif' activity would be significantly more complex: adjust the
    ...
    def __init__(self) -> None:
        """Construct the node."""
        super().__init__('task_notif_task')
        ...
        self.current_greeting_status = Notif.Goal().INACTIVE
        self._get_result_future = None

        self.target_user_id = None
        self.custom_greetings = None

        self.get_logger().info('Task notif_task started, but not yet configured.')
    ...
    def on_request_exec(self, goal_handle):
        ...
        if self.target_user_id:
            self.current_notif_status = Notif.Goal().Danger_Notif
            self.get_logger().info(f"notification to user <{self.target_user_id}> for bad posture.")
        else:
            self.current_greeting_status = Greet.Goal().SELECTING_USER
            self.get_logger().info("Starting by selecting a user to greet.")

        feedback_msg = Notif.Feedback()
    ...
    def greet_goal_callback(self, future):
        ...
        if not goal_handle.accepted:
            self.get_logger().info("Failed to greet the user. '/say' goal rejected.")
            self.current_greeting_status = Greet.Goal().INACTIVE
            return
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        self.get_logger().info("Task greet_task is configured, but not yet active")
    ...
    def on_activate(self, state: State) -> TransitionCallbackReturn:
        ...
        self.get_logger().info("Task greet_task is active and running")
    ...
    def on_deactivate(self, state: State) -> TransitionCallbackReturn:
        ...
        self.get_logger().info("Task greet_task is stopped (inactive)")
    ...
    def on_shutdown(self, state: State) -> TransitionCallbackReturn:
        ...
        :return: The state machine either invokes a transition to the
            ...
            "errorprocessing"
        """
        self.get_logger().info('Shutting down task greet_task.')
        self.destroy_timer(self._diag_timer)
        self.destroy_publisher(self._diag_pub)

        self.destroy_timer(self._timer)

        self.get_logger().info("Task greet_task finalized.")
        return TransitionCallbackReturn.SUCCESS
    ...
    def publish_diagnostics(self):
        ...
        msg = DiagnosticStatus(
            ...
            name="/tasks/task_greet_task",
            message="task greet_task is running",
            values=[
                KeyValue(key="Module name", value="greet_task"),
