...
To implement your skill logic, have a look at `say_skill/skill_impl.py`: this is
...
- run `ros2 run pal_deploy deploy --package say_skill generic-XXc` (replace `generic-XXc` by your actual robot)

You can now `ssh` onto the robot (`ssh pal@generic-XXc`, password
`pal`), and go to your project: `cd ~/deployed_ws/share/say_skill`
...
`ros2 run say_skill start_skill`
...
``ros2 lifecycle set /app_say_skill configure``
``ros2 lifecycle set /app_say_skill activate``

To stop it (but not reseting it's internal variables):
``ros2 lifecycle set /app_say_skill activate``

To stop it and clean it:
``ros2 lifecycle set /app_say_skill deactivate``
``ros2 lifecycle set /app_say_skill cleanup``
